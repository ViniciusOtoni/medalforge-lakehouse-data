...
def test_run_pipeline_feliz_com_sink(monkeypatch, spark):
    ...
    Verificamos:
      - persist_df_append_external chamado para quarentena bruta e para 'rejected' com cat/schema do sink
    ...
    class FakeSpark(spark.__class__):
        def table(self, name):
            assert name == "bronze.sales.orders"
            return bronze_df
...
def test_run_pipeline_sem_sink_cria_rejected_no_schema_quarantine(monkeypatch, spark):
    ...
    # dqx: tudo inválido vai para 'still_bad' no recheck para disparar o caminho do rejected
    def fake_initial_split(df, _):  # tudo válido inicialmente (simplifica)
        return df, df.limit(0)

    def fake_recheck(df, _):  # simula que remediação falhou (tudo ainda ruim)
    ...
    # Apenas uma persistência: rejected (não houve quarentena bruta porque initial_split deu vazio)
    assert len(seen["tables"]) == 1
    assert seen["tables"][0] == "silver.sales_quarantine.orders_rejected"
